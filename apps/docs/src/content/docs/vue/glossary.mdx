...
import { Tabs, TabItem } from '@astrojs/starlight/components';
  ...
  An async component is a wrapper around another component that allows for the wrapped component to be lazy loaded. 
  
  This is typically used as a way to reduce the size of the built <Badge text=".js" variant="note" /> files, allowing them to be split into smaller chunks that are loaded only when required.
...
## compiler macro
  ...
  Vue's SFC compiler supports various macros, such as defineProps(), defineEmits() and defineExpose(). 
  ...
  However, they are not actual functions that are run in the browser. These are special strings that the compiler detects and replaces with the real JavaScript code that will actually be run.
