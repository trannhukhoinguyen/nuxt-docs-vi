...
import { Tabs, TabItem } from '@astrojs/starlight/components';
  ...
  Bảng thuật ngữ này nhằm mục đích cung cấp một số hướng dẫn về ý nghĩa của các thuật ngữ kỹ thuật thường được sử dụng khi nói về Vue. 
  
  Bảng thuật ngữ này nhằm mục đích mô tả cách các thuật ngữ thường được sử dụng, chứ không phải là một quy định bắt buộc về cách sử dụng chúng. 
  
  Một số thuật ngữ có thể có ý nghĩa hoặc sắc thái hơi khác nhau tùy thuộc vào ngữ cảnh xung quanh.
  
  ## async component

  Thành phần bất đồng bộ là một lớp bao bọc xung quanh một thành phần khác, cho phép thành phần được bao bọc này được tải lười biếng.
    
  Điều này thường được sử dụng như một cách để giảm kích thước của các tệp <Badge text=".js" variant="note" /> đã xây dựng, cho phép chúng được chia thành các phần nhỏ hơn và chỉ được tải khi cần thiết.
  
  Vue Router có một tính năng tương tự để <a href="/guide/advanced/lazy-loading" target="_blank"><span style="color: #00dc82">tải lười biếng các thành phần tuyến đường</span></a>, mặc dù tính năng này không sử dụng tính năng thành phần bất đồng bộ của Vue.

  Xem thêm tại:
  
  - <a href="/guide/components/async" target="_blank"><span style="color: #00dc82">Guide - Async Components</span></a>

## compiler macro

  Macro biên dịch là mã đặc biệt được trình biên dịch xử lý và chuyển đổi thành thứ khác.
  Chúng thực sự là một dạng thay thế chuỗi thông minh.

  Trình biên dịch <a href="/vue/glossary/#single-file-component" target="_blank"><span style="color: #00dc82">SFC</span></a> của Vue hỗ trợ nhiều macro khác nhau, chẳng hạn như <Badge text="defineProps()" variant="note" />, <Badge text="defineEmits()" variant="note" /> và <Badge text="defineExpose()" variant="note" />. 
  Các macro này được thiết kế có chủ đích để trông giống như các hàm JavaScript thông thường, nhằm tận dụng cùng một trình phân tích cú pháp và công cụ suy luận kiểu dữ liệu xung quanh JavaScript/TypeScript. 
  Tuy nhiên, chúng không phải là các hàm thực sự được chạy trên trình duyệt. 
  Chúng là những chuỗi ký tự đặc biệt mà trình biên dịch sẽ phát hiện và thay thế bằng mã JavaScript thực sự sẽ được chạy.

  Macro có những hạn chế về cách sử dụng mà không áp dụng cho mã JavaScript thông thường. Ví dụ, bạn có thể nghĩ rằng <Badge text="const dp = defineProps" variant="note" /> sẽ cho phép bạn tạo một bí danh cho defineProps, nhưng thực tế nó sẽ gây ra lỗi. 
  Cũng có những hạn chế về giá trị có thể được truyền vào <Badge text="defineProps()" variant="note" />, vì 'đối số' phải được trình biên dịch xử lý chứ không phải trong thời gian chạy.
    
  Xem thêm tại:

  - <a href="/vue/api/sfc-script-setup/#defineprops-defineemits" target="_blank"><span style="color: #00dc82">&lt;script setup&gt; - defineProps() & defineEmits()</span></a>
  - <a href="/vue/api/sfc-script-setup/#defineexpose" target="_blank"><span style="color: #00dc82">&lt;script setup&gt; - defineExpose()</span></a>

## component

Thuật ngữ "thành phần" không chỉ dành riêng cho Vue. Nó phổ biến trong nhiều framework UI. Nó mô tả một phần của UI, chẳng hạn như nút bấm hoặc hộp kiểm. Các thành phần cũng có thể được kết hợp để tạo thành các thành phần lớn hơn.

Thành phần là cơ chế chính được Vue cung cấp để chia nhỏ UI thành các phần nhỏ hơn, vừa để cải thiện khả năng bảo trì vừa cho phép tái sử dụng mã.

Thành phần Vue là một đối tượng. Tất cả các thuộc tính đều là tùy chọn, nhưng cần có mẫu hoặc hàm render để thành phần có thể render. 

Ví dụ: đối tượng sau sẽ là một thành phần hợp lệ:
  ...
  Trên thực tế, hầu hết các ứng dụng Vue đều được viết bằng <a href="/vue/glossary#single-file-component" target="_blank"><span style="color: #00dc82">Single-File Components</span></a> (tệp <Badge text=".vue" variant="note" />). 
  Mặc dù thoạt nhìn các thành phần này có vẻ không phải là đối tượng, nhưng trình biên dịch SFC sẽ chuyển đổi chúng thành một đối tượng, được sử dụng làm tệp xuất mặc định cho tệp. 
  Từ góc nhìn bên ngoài, tệp <Badge text=".vue" variant="note" /> chỉ là một mô-đun ES xuất một đối tượng thành phần.
  
  Các thuộc tính của một đối tượng thành phần thường được gọi là tùy chọn. Đây chính là nguồn gốc tên gọi của <a href="/vue/glossary/#options-api" target="_blank"><span style="color: #00dc82">API Tùy chọn</span></a>.
  
  Các tùy chọn cho một thành phần xác định cách tạo các thể hiện của thành phần đó. Về mặt khái niệm, các thành phần tương tự như các lớp, mặc dù Vue không sử dụng các lớp JavaScript thực tế để định nghĩa chúng.
  
  Thuật ngữ thành phần cũng có thể được sử dụng rộng rãi hơn để chỉ các thể hiện của thành phần.

  Xem thêm tại:

  - <a href="/vue/guide/essentials/component-basics" target="_blank"><span style="color: #00dc82">Guide - Component Basics</span></a>

  Từ <Badge text="component" variant="note" /> cũng có nhiều thuật ngữ khác:

  - <a href="/vue/glossary/#async-component" target="_blank"><span style="color: #00dc82">async component</span></a>
  - <a href="/vue/glossary/#dynamic-component" target="_blank"><span style="color: #00dc82">dynamic component</span></a>
  - <a href="/vue/glossary/#functional-component" target="_blank"><span style="color: #00dc82">functional component</span></a>
  - <a href="/vue/glossary/#web-component" target="_blank"><span style="color: #00dc82">Web Component</span></a>
...
## fragment
  ...
  Fragments are also used by the template compiler as a way to wrap multiple dynamic nodes, e.g. those created via v-for or v-if. This allows for extra hints to be passed to the VDOM patching algorithm. Much of this is handled internally, but one place you may encounter this directly is using a key on a `<template>` tag with v-for. 
  
  In that scenario, the key is added as a prop to the fragment VNode.
...
## cache static
  ...
  - Guide - Rendering Mechanism - Cache Static
...
## virtual DOM
  ...
  Rather than creating DOM nodes directly, Vue components generate a description of what DOM nodes they would like. 
  
  These descriptors are plain JavaScript objects, known as VNodes (virtual DOM nodes). Creating VNodes is relatively cheap.
  ...
  Vue uses a hybrid approach that we call Compiler-Informed Virtual DOM. Vue's template compiler is able to apply performance optimizations based on static analysis of the template. 
  
  Rather than performing a full comparison of a component's old and new VNode trees at runtime, Vue can use information extracted by the compiler to reduce the comparison to just the parts of the tree that can actually change.
...
## Web Component
  ...
  Tiêu chuẩn Web Components là tập hợp các tính năng được triển khai trong các trình duyệt web hiện đại. 
  
  Các thành phần Vue không phải là Web Components, nhưng defineCustomElement() có thể được sử dụng để tạo một <a href="/vue/glossary#custom-element"><span style="color: #00dc82">phần tử tùy chỉnh</span></a> từ một thành phần Vue. 
  
  Vue cũng hỗ trợ việc sử dụng các phần tử tùy chỉnh bên trong các thành phần Vue.

  Để biết thêm chi tiết, hãy xem:

  <a href="/vue/guide/extras/web-components" target="_blank"><span style="color: #00dc82">Hướng dẫn - Vue và Web Components</span></a>
